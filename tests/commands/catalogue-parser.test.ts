...
describe('parser', () => {
  ...
  const barcodesBUnique = [
    { SupplierID: '00001', SKU: '947-vyk-317', Barcode: 'z2783613083810' },
    { SupplierID: '00002', SKU: '647-vyk-317', Barcode: 'z2783613083818' }
  ];
  const suppliersA = [
  ...
  const catalogB = [
    { SKU: '647-vyk-317', Description: 'Another Walkers Special Old Whiskey' },
  ...
  it('should give correct filtered down results without suppliers', () => {
    const results = parse(barcodesA, barcodesB, catalogA, catalogB, [], []);
    expect(results.length).toBe(2);
    expect(results.map(res => res['Source'])).toContain('B');
  });

  it('should give all unique items', () => {
    const results = parse(
      barcodesA,
      barcodesBUnique,
      catalogA,
